...
## **Reparameterization: Defining the GAL in Terms of γ and p₀**
...
df <- data.frame(
  ...
  gamma = factor(rep(c("γ = 0.5", "γ = -0.5", "γ = 0.1"), each = length(x)))
)
...
ggplot(df, aes(x = x, y = y, color = gamma)) +
  geom_line(size = 1.2) +
  scale_color_manual(values = c("#1f78b4", "#33a02c", "#e31a1c")) +  # Professional color palette
  labs(title = "GAL Density for Different γ Values at quantile level p0 = 0.6",
       x = "x",
       y = "Density",
       color = "γ Values | ") +
  theme_minimal(base_size = 14) +
...
Using the **relationship between $(p, \alpha)$ and $(p_0, \gamma)$**, the GAL **density function** can be expressed in terms of **gamma and p₀**.
...
ggplot(df, aes(x = x, y = y, color = gamma)) +
  geom_line(size = 1.2) +
  scale_color_manual(values = c("#1f78b4", "#33a02c", "#e31a1c")) +  # Professional color palette
  labs(title = "GAL CDF for Different γ Values at Quantile Level p0 = 0.6",
       x = "x",
       y = "Cumulative Probability",
       color = "γ Values | ") +
  theme_minimal(base_size = 14) +
...
df <- data.frame(
  ...
  gamma = factor(rep(c("γ=0.5, μ=2, σ=1.5", 
                        "γ=-0.5, μ=-3, σ=2", 
                        "γ=0.1, μ=0, σ=1"), 
                      each = length(x)))
)
...
ggplot(df, aes(x = x, y = y, color = gamma)) +
  geom_line(size = 1.2) +
  scale_color_manual(values = c("#1f78b4", "#33a02c", "#e31a1c")) +  # Professional color palette
  labs(title = "GAL Density for Different γ, μ, and σ Values at p0 = 0.6",
       x = "x",
       y = "Density",
       color = "γ, μ, σ Values | ") +
  theme_minimal(base_size = 14) +
...
ggplot(df, aes(x = x, y = y, color = gamma)) +
  geom_line(size = 1.2) +
  scale_color_manual(values = c("#1f78b4", "#33a02c", "#e31a1c")) +  # Professional color palette
  labs(title = "Generalized Asymmetric Laplace CDF for Different γ, μ, and σ Values at p0 = 0.6",
       x = "x",
       y = "Cumulative Probability",
       color = "γ, μ, σ Values | ") +
  theme_minimal(base_size = 14) +
...
# The GAL parameter γ is constrained by two bounds, L and U, which depend on p_0.
# These bounds define the valid region for γ at a given p_0.
...
ggplot(gamma_bounds, aes(x = p0)) +
  geom_ribbon(aes(ymin = L, ymax = U), fill = "lightblue", alpha = 0.4) +  # Shaded region for valid γ
  ...
  labs(title = "Valid Range of γ for Different p0 Values",
       x = "p0",
       y = "γ Bounds",
       color = " Bounds ") +
  theme_minimal(base_size = 14) +
...
for (case in test_cases) {
  ...
  case_label <- paste0("p0=", p0, ", γ=", gamma, ", μ=", mu)
...
ggplot(df_plot, aes(x = p, y = p_recovered)) +
  ...
  labs(title = "Validation of qexal as the Inverse of pexal",
       subtitle = "Each panel corresponds to a different (p0, γ, μ) configuration",
